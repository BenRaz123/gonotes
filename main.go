...
func (s server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if isDir {
		data = newPageDataForDir(leadingDir, files, dirs)
	} else {
		...
		data = newPageDataForFile(leadingDir, pathpkg.Base(path), string(html), files, dirs)
	}
...
func main() {
	////d := newPageDataForDir([]string{"poop", "crap", "shit", "faeces"}, []string{"ababab.md", "popopoy.md", "crapalooloo.md"}, []string{"a", "b", "c"})
	//d := newPageDataForFile([]string{"poop", "crap", "shit", "faeces"}, "Crap.Md", "<h1>Hello World</h1>\n<p>Hello</p><ul><li>Hello</li><li>World</li></ul>", []string{"ababab.md", "popopoy.md", "crapalooloo.md"}, []string{"a", "b", "c"})
	//html, err := d.Render()
	//if err != nil {
	//	log.Fatal(err)
	//}
	//if err := os.WriteFile("poop.html", []byte(html), 0644); err != nil {
	//	log.Fatal(err)
	//}
